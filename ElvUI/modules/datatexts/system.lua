	...
	if IsShiftKeyDown() or not cpuProfiling then
		...
		for i = 1, #memoryTable do
			if (memoryTable[i][4]) then
				...
				DT.tooltip:AddDoubleLine(memoryTable[i][2], formatMem(memoryTable[i][3]), 1, 1, 1, red, 1- red + .5, 0)
			end						
		end
	end
	...
	if cpuProfiling and not IsShiftKeyDown() then
		...
		for i = 1, #cpuTable do
			if (cpuTable[i][4]) then
				...
				DT.tooltip:AddDoubleLine(cpuTable[i][2], format(homeLatencyString, cpuTable[i][3]), 1, 1, 1, red, 1- red + .5, 0)
			end						
		end
		DT.tooltip:AddLine(" ")
